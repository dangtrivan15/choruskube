...
describe("useRunSubscription", () => {
  ...
  it("invalidates live-chat query on live_chat_status_changed completed event", () => {
    const { wrapper, queryClient } = createTestHookWrapper();

    // Pre-populate the live-chat query cache
    queryClient.setQueryData(["live-chat", "run-1", "exec-1"], {
      id: "session-1",
      status: "active",
    });
    expect(queryClient.getQueryData(["live-chat", "run-1", "exec-1"])).toBeDefined();

    const invalidateSpy = vi.spyOn(queryClient, "invalidateQueries");

    renderHook(() => useRunSubscription("run-1"), { wrapper });

    // Get the subscription callback
    const subscribeCallback = mockSubscribe.mock.calls[0][1];

    // Simulate a live_chat_status_changed event with completed status
    subscribeCallback({
      body: JSON.stringify({
        type: "live_chat_status_changed",
        runId: "run-1",
    ...
    // The live-chat query should be invalidated (not removed) so the
    // completed session data can be re-fetched and displayed
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: ["live-chat", "run-1", "exec-1"],
    });
  });

  it("invalidates live-chat query on live_chat_status_changed failed event", () => {
    const { wrapper, queryClient } = createTestHookWrapper();
    ...
    // The live-chat query should be invalidated (not removed)
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: ["live-chat", "run-1", "exec-1"],
    });
  });

  it("does not remove live-chat query for non-terminal live_chat_status_changed event", () => {
    const { wrapper, queryClient } = createTestHookWrapper();
    ...
    // "active" status should NOT remove the cache
    subscribeCallback({
      body: JSON.stringify({
        type: "live_chat_status_changed",
        runId: "run-1",
