...
class DefaultRoadmapCandidateMaterializerTest {
    ...
    private static EpicResponse epicResponse(UUID id) {
        return new EpicResponse(id, "t", "d", "m", "open", "active", null, null, null, Instant.now(), Instant.now());
    }

    private static StoryResponse storyResponse(UUID id, UUID epicId) {
        return new StoryResponse(id, epicId, "t", "d", "open", null, null, Instant.now(), Instant.now());
    }
