...
| [2026-09-21---01-roadmap-edge-style-registry.md](2026-09-21---01-roadmap-edge-style-registry.md) | 2026-09-21 | Why every roadmap dependency-edge color/dash-pattern consumer (stroke, both marker call sites, legend swatch) reads one shared `ROADMAP_EDGE_STYLES` registry keyed by the raw `--status-*` token instead of each carrying its own literal | current |
