...
// TestEpochResetOnHumanBackEdge verifies that when a human node routes back to an AI node,
// the new AI execution gets iterationCapEpochStart = newIteration (epoch reset).
func (s *DAGExecutorTestSuite) TestEpochResetOnHumanBackEdge() {
	aiNode := uuid.New()
	humanNode := uuid.New()
	execAI1 := uuid.New()
	execHuman := uuid.New()
	...
	snapshot := `{
        "nodes": [
            {"templateNodeId": "` + aiNode.String() + `", "label": "AI", "executorType": "ai", "timeoutSeconds": 1800, "isEntrypoint": true},
            {"templateNodeId": "` + humanNode.String() + `", "label": "Human", "executorType": "human", "timeoutSeconds": 3600}
        ],
        "edges": [
            {"templateEdgeId": "` + uuid.New().String() + `", "sourceNodeId": "` + aiNode.String() + `", "targetNodeId": "` + humanNode.String() + `", "condition": "need_human_decision:iteration_cap"},
            {"templateEdgeId": "` + uuid.New().String() + `", "sourceNodeId": "` + humanNode.String() + `", "targetNodeId": "` + aiNode.String() + `", "condition": "rereview"}
        ]
    }`
	...
	// AI node 1: entry
	s.env.OnActivity("CreateNodeExecution", mock.Anything, mock.MatchedBy(func(p activity.CreateNodeExecParams) bool {
		return p.TemplateNodeID == aiNode && p.Iteration == 1
	})).Return(execAI1, nil).Once()

	// AI node executes and routes to human
	s.env.OnActivity("ExecuteAINodeFromSnapshot", mock.Anything, mock.MatchedBy(func(p activity.ExecuteAINodeFromSnapshotParams) bool {
		return p.TemplateNodeID == aiNode
	})).Return(nil).Once()

	s.env.OnActivity("GetNodeDecision", mock.Anything, mock.MatchedBy(func(p activity.GetNodeDecisionParams) bool {
		return p.NodeExecutionID == execAI1
	})).Return("need_human_decision:iteration_cap", nil).Once()

	// Human node created
	s.env.OnActivity("CreateNodeExecution", mock.Anything, mock.MatchedBy(func(p activity.CreateNodeExecParams) bool {
		return p.TemplateNodeID == humanNode && p.Iteration == 1
	})).Return(execHuman, nil).Once()

	// Human signal: rereview
	s.env.RegisterDelayedCallback(func() {
		s.env.SignalWorkflow(SignalHumanDecisionPrefix+execHuman.String(), HumanDecisionSignal{Decision: "rereview"})
	}, time.Millisecond*100)

	s.env.OnActivity("GetNodeDecision", mock.Anything, mock.MatchedBy(func(p activity.GetNodeDecisionParams) bool {
		return p.NodeExecutionID == execHuman
	})).Return("rereview", nil).Once()

	// AI node 2: epoch reset — iterationCapEpochStart MUST equal 2
	s.env.OnActivity("CreateNodeExecution", mock.Anything, mock.MatchedBy(func(p activity.CreateNodeExecParams) bool {
	...
	s.env.OnActivity("GetNodeDecision", mock.Anything, mock.MatchedBy(func(p activity.GetNodeDecisionParams) bool {
		return p.NodeExecutionID == execAI2
	})).Return("approved", nil).Once()
...
// TestEpochCarriedForwardOnAIBackEdge verifies that on AI→AI self-loop back-edges,
// iterationCapEpochStart is carried forward unchanged (not reset).
func (s *DAGExecutorTestSuite) TestEpochCarriedForwardOnAIBackEdge() {
	aiNode := uuid.New()
	termNode := uuid.New()
	exec1 := uuid.New()
	exec2 := uuid.New()
	...
	snapshot := `{
        "nodes": [
            {"templateNodeId": "` + aiNode.String() + `", "label": "AI", "executorType": "ai", "timeoutSeconds": 1800, "isEntrypoint": true},
            {"templateNodeId": "` + termNode.String() + `", "label": "Term", "executorType": "ai", "timeoutSeconds": 1800}
        ],
        "edges": [
            {"templateEdgeId": "` + uuid.New().String() + `", "sourceNodeId": "` + aiNode.String() + `", "targetNodeId": "` + aiNode.String() + `", "condition": "revised"},
            {"templateEdgeId": "` + uuid.New().String() + `", "sourceNodeId": "` + aiNode.String() + `", "targetNodeId": "` + termNode.String() + `", "condition": "approved"}
        ]
    }`
	...
	// AI iteration 1 (epoch_start=1)
	s.env.OnActivity("CreateNodeExecution", mock.Anything, mock.MatchedBy(func(p activity.CreateNodeExecParams) bool {
		return p.TemplateNodeID == aiNode && p.Iteration == 1
	})).Return(exec1, nil).Once()
	...
	s.env.OnActivity("GetNodeDecision", mock.Anything, mock.MatchedBy(func(p activity.GetNodeDecisionParams) bool {
		return p.NodeExecutionID == exec1
	})).Return("revised", nil).Once()

	// AI iteration 2: self-loop from AI node, epochStart MUST still be 1 (carried forward)
	s.env.OnActivity("CreateNodeExecution", mock.Anything, mock.MatchedBy(func(p activity.CreateNodeExecParams) bool {
	...
	s.env.OnActivity("GetNodeDecision", mock.Anything, mock.MatchedBy(func(p activity.GetNodeDecisionParams) bool {
		return p.NodeExecutionID == exec2
	})).Return("approved", nil).Once()

	// Terminal node
	s.env.OnActivity("CreateNodeExecution", mock.Anything, mock.MatchedBy(func(p activity.CreateNodeExecParams) bool {
		return p.TemplateNodeID == termNode
	})).Return(execTerm, nil).Once()
	...
	s.env.OnActivity("GetNodeDecision", mock.Anything, mock.MatchedBy(func(p activity.GetNodeDecisionParams) bool {
		return p.NodeExecutionID == execTerm
	})).Return("no_decision", nil).Once()
