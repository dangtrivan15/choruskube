...
- Agent-facing CLI scripts (on PATH in the agent image): `report-result`, `fetch-github-token`, `check-decision`, `list-decisions`, `artifact`, `create-proposal`, `list-proposals`, `update-proposal`, `register-pr`, `run-all-tests`
...
- Custom enum types exist: `executor_type`, `workflow_run_status`, `node_execution_status`, `log_level`, `reviewer_type`, `provisioning_status`, `live_chat_status`, `work_item_status`, `invitation_status`, `github_credential_health_status` — add new values with `ALTER TYPE ... ADD VALUE`
